...
function parseDeviceInfo(req) {
    ...
    if (!result?.browser?.version) result.browser.version = {};
    if (!result?.os?.version) result.os.version = {};
    ...
    return {
        ...
        os: {
            name: result?.os?.name,
            version: result?.os?.version?.value,
        },
        // Check for device data in request,
        // use that to override guessing browser data.
        ...(typeof deviceDataInBody === 'object' && deviceDataInBody !== null ? { ...deviceDataInBody } : {}),
    };
}
