...
// Aggregate all data from the incidents collection
const aggregateIncidents = [
    {
        $group: {
            _id: "incidents",
            total: {
...
// Most & least common Event (Error or Cutstom Event)
const aggregateModeEvent = (which) => {
    // Match error events
    let eventMatch = {
        event_type: "error",
    };

    // Match anything that is NOT an error (custom events)
    if (which !== "error") {
        eventMatch = {
            event_type: { $ne: "error" },
        };
